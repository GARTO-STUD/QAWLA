...
export async function POST(req: Request) {
  ...
  try {
    const result = await runAnalyst(body.events, confidence);
    return NextResponse.json({ result }, { headers: rateLimitHeaders(rl) });
  } catch (err) {
    return NextResponse.json(
      { error: 'Analyst agent failed', detail: err instanceof Error ? err.message : String(err) },
...
export async function GET(req: Request) {
  ...
  return NextResponse.json({
    agent: 'analyst',
    description: 'Produces deep tactical breakdowns: formations, pressing schemes, key battles, statistical insights.',
    input: { events: 'RawEvent[]', confidence: 'ConfidenceResult (optional)' },
    output: 'TacticalReport',

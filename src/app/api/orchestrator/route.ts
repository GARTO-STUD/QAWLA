...
export async function POST(req: Request) {
  ...
  let body: { trigger?: PipelineJob['trigger']; sourceIds?: string[]; skipPublish?: boolean; topic?: string } = {};
  try {
    body = await req.json();
  } catch {
  ...
  const topic = typeof body.topic === 'string' ? body.topic.trim().slice(0, 300) : undefined;
  const ctx: OrchestratorContext = {
      job,
      events: allEvents.slice(0, 8),
      editorialFocus: topic || undefined,
      sources,
    };

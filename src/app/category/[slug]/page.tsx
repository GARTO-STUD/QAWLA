...
export default async function CategoryPage({ params }: PageProps) {
  ...
  if (!cat) {
    return (
      <PageHero title="Category not found" variant="light">
        ...
        <div className="flex gap-2 overflow-x-auto pb-3 mb-8 -mx-4 px-4 sm:mx-0 sm:px-0">
          ...
          {CATEGORIES.map((c) => (
            <Link
              ...
              className={`badge whitespace-nowrap flex-shrink-0 transition-colors ${c.slug === slug ? 'bg-pitch text-night' : 'bg-white border border-gray-200 text-night/70 hover:border-pitch hover:text-pitch-dk'}`}
            >
              {c.label}
            </Link>
          ))}

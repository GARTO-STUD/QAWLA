...
export default async function TagPage({ params }: PageProps) {
  const { slug } = await params;
  const tag = decodeURIComponent(slug);
  const articles = ARTICLES.filter((a) => a.tags.includes(tag) && a.status === 'published');
          ...
          <div className="flex flex-wrap gap-2">
            {Array.from(new Set(ARTICLES.flatMap((a) => a.tags))).slice(0, 20).map((t) => (
              <Link
                key={t}
                href={`/tag/${t}`}
                className={`badge transition-colors ${t === tag ? 'bg-pitch text-night' : 'bg-gray-100 text-night/70 hover:bg-pitch/10 hover:text-pitch-dk'}`}
              >
                #{t}
              </Link>
            ))}

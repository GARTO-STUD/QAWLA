...
export function ArticleCard({
  ...
  if (variant === 'compact') {
    return (
      <Link
        ...
        className={cn(
          'group flex gap-3 p-3 rounded-sm hover:bg-pitch/5 transition-colors',
          className,
        )}
      >
        {article.coverImage && (
      ...
      >
        ...
        <div className="p-4 sm:py-5 sm:pr-5">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            ...
            {article.imageVerified ? (
              <span className="badge bg-white/90 text-night">Source image</span>
            ) : article.coverImage ? (
              <span className="badge bg-night/70 text-white backdrop-blur">Topic match</span>
            ) : null}
            {article.trending && (
  ...
  if (variant === 'featured') {
    return (
      <Link
        href={`/news/${article.id}`}
        className={cn(
          'group relative block rounded-3xl overflow-hidden qawla-card min-h-[400px] sm:min-h-[480px] bg-night',
          className,
        )}
      >
  ...
  return (
    <Link
      href={`/news/${article.id}`}
      className={cn('group flex flex-col qawla-card shine-sweep', className)}
    >

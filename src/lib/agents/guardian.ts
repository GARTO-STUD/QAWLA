...
Respond with JSON matching this schema:
{
  ...
  if (report.overrideDecision === 'reject') {
    ...
    if (article) {
      const VALID_STATUSES: Article['status'][] = ['draft', 'in_review', 'fact_checking', 'published', 'archived', 'rejected'];
      for (const fix of fixes) {
        ...
        if (fix.field === 'status' && fix.newValue) {
          // Never trust an LLM-produced string directly for a field that
          // gates what gets published. The only current caller (runPipeline)
          // re-asserts article.status from the confidence-derived
          // policyDecision right after this function returns, so this isn't
          // exploitable today — but that safety net lives in a different
          // file, and any future standalone caller of runGuardian (e.g. a
          // dedicated /api/agents/guardian route, mirroring the other five
          // agents which each already have one) would apply this unchecked
          // cast with no re-gating at all.
          if ((VALID_STATUSES as string[]).includes(fix.newValue)) {
            article.status = fix.newValue as Article['status'];
          }
        }
      }
    }

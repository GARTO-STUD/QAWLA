...
const COLLECTIONS_TO_BACKUP: { name: string; limit: number; orderBy?: string }[] = [
  { name: 'articles', limit: 1000 },
  { name: 'raw_events', limit: 500, orderBy: 'publishedAt' },
  { name: 'credibility_sources', limit: 100 },
  { name: 'jobs', limit: 200, orderBy: 'createdAt' },
...
export async function createBackup(trigger: 'cron' | 'manual' = 'manual'): Promise<BackupMetadata> {
  ...
  for (const { name, limit, orderBy } of COLLECTIONS_TO_BACKUP) {
    try {
      const result = await listDocs<unknown>(name, {
        pageSize: limit,
        ...(orderBy ? { orderBy, orderDirection: 'DESCENDING' as const } : {}),
      });
 ...
 *
 * Deliberately does NOT reuse listBackups(), which caps its query at
 * `pageSize: MAX_BACKUPS` for display purposes — reusing it here made this
 * function's own length check (`backups.length <= MAX_BACKUPS`) always true,
 * since it could never observe more than MAX_BACKUPS backups in the first
 * place. That meant cleanup never ran and backups piled up indefinitely
 * despite the documented "keeps only the last 30" behavior. This fetches a
 * larger page explicitly so it can actually detect and trim any surplus.
 */
async function cleanupOldBackups(): Promise<void> {
  try {
    const result = await listDocs<BackupSnapshot>(BACKUP_COLLECTION, { pageSize: MAX_BACKUPS * 4 });

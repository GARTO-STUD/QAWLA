...
import { runSEOAgent, type SEOReport } from '@/lib/agents/seo';
import { resolveEventImage } from '@/lib/articleImage';
...
function synthesizeArticle(events: RawEvent[], confidence: ConfidenceResult): Article {
  ...
  return {
    ...
    imageSourceName: lead?.imageSourceName,
    imageVerified: lead?.imageVerified,
 ...
 *
 * Stages: scout → fact_check → analyst → writer → editor → seo → publish → complete
...
export async function runPipeline(
  ...
  const { job } = ctx;
  // Resolve images before any AI agent sees the event. The pipeline is source-first:
  // an RSS enclosure/media image or the source article's og:image wins over any
  // generic football artwork.
  const sourceMap = new Map((ctx.sources ?? DEFAULT_SOURCES).map((source) => [source.id, source]));
  const events = await Promise.all(
    ctx.events.map((event) => resolveEventImage(event, sourceMap.get(event.sourceId))),
  );
  const editorialFocus = ctx.editorialFocus;
  ...
  job.stage = 'scout';
  try {
    const scoutResult = await runScout(events, confidence, editorialFocus);
  ...
  // 7. SEO Intelligence — analyzes the final edited article before publish.
  // SEO failure is non-fatal; the report is attached to the job for editorial
  // observability and can be persisted later for learning from Search Console.
  try {
    const seoResult = await runSEOAgent(article);
    markResult(job, seoResult, 'publish');
    if (seoResult.status === 'completed' && seoResult.output) {
      const seo = seoResult.output as SEOReport;
      // Keep SEO recommendations attached without changing factual content.
      article.seo = {
        title: seo.seoTitle,
        ...
        score: seo.score,
        newsReadiness: seo.newsReadiness,
        recommendations: seo.recommendations,
      };
    }
  } catch (err) {
    markResult(job, failResult('seo', err), 'publish');
  }

  // 8. Guardian — silent supervisor (best-effort, never blocks)
  try {

 ...
 *
 * Prefers GEMINI_GUARDIAN_API_KEY (a separate key/project) so Guardian's quota
 * is truly isolated from the other agents' Nvidia→Groq→Gemini waterfall,
 * which also falls back to GEMINI_API_KEY under load. Sharing one key would
 * silently defeat the whole point of giving Guardian its own provider: both
 * would compete for the same free-tier quota bucket. Falls back to
 * GEMINI_API_KEY for backward compatibility if no dedicated key is set.
...
export async function callGeminiDirect(
  ...
  const apiKey = process.env.GEMINI_GUARDIAN_API_KEY ?? process.env.GEMINI_API_KEY;
  if (!apiKey) {
    ...
    return {
      ...
      content: JSON.stringify({
        ...
        rationale: 'Guardian running in offline mode (no GEMINI_GUARDIAN_API_KEY/GEMINI_API_KEY). No issues detected by rule-based checks.',

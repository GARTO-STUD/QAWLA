...
function requireAuth(req: Request): { ok: true; rl: ReturnType<typeof rateLimit> } | { ok: false; response: Response } {
  ...
  const session = token ? null : null; // verifySession is async — handle below
  void session;
  return { ok: true, rl };
}
